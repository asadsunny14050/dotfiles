...
M.executor = function(cmd, cwd)
    ...
    -- Create the split window first
    vim.cmd("botright split")
    M.compile_window = vim.api.nvim_get_current_win()

    -- Set the buffer in the window
    vim.api.nvim_win_set_buf(M.compile_window, compile_buffer)
    ...
    vim.fn.jobstart(cmd_table, {
        ...
        on_stdout = function(_, data)
            if data then
                vim.schedule(function()
                    vim.api.nvim_buf_set_lines(compile_buffer, -1, -1, false, data)
                end)
            end
        end,
        ...
        on_exit = function(_, code)
            vim.schedule(function()
                local status_line = "[Command finished with code " .. code .. "]"
                vim.api.nvim_buf_set_lines(compile_buffer, -1, -1, false, { "", status_line })
            end)
        end,
